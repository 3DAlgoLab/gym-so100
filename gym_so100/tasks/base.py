...
from gym_so100.tasks import mocap
...
class Base(gym.Env):
    ...
    def __init__(
        ...
        self.visualization_renderer = self._initialize_renderer(
            renderer_type="visualization"
        )
        self.observation_space = self._initialize_observation_space()
        self.action_space = gym.spaces.Box(
            low=-1.0, high=1.0, shape=(len(self.metadata["action_space"]),)
        )
        self.action_padding = np.zeros(
            4 - len(self.metadata["action_space"]), dtype=np.float32
        )
    ...
    def _initialize_observation_space(self):
        ...
        if self.obs_type == "state":
            observation_space = gym.spaces.Box(
                -1000.0, 1000.0, shape=obs.shape, dtype=np.float64
            )
